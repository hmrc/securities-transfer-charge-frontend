  ...
  def fakeNavigatorWithValidatorOutcome(outcome: Either[Call, Boolean]): Navigator = new Navigator {
    ...
    override val userAnswersValidator: UserAnswersValidator = new UserAnswersValidator(this)(ec) {
      ...
      override protected val startPage: GettablePage[?] = outcome match {
        ...
        case Left(_)      => AgentReferencePage
      }

      override protected lazy val pageCallMap: PageCallBiMap = outcome match {
        case Left(call) =>
          new PageCallBiMapBuilder().addMapping(AgentReferencePage, _ => call).build
        case _ =>
    ...
    "onPageLoad" - {
      ...
      "must return OK and the correct view for a GET when validation succeeds (Right(true))" in {
        val application = applicationBuilder(userAnswers = Some(emptyUserAnswers))
          .overrides(
            bind[Navigator].qualifiedWith(Names.named("agentsSh03")).toInstance(fakeNavigatorWithValidatorOutcome(Right(true)))
          )
          .build()
      ...
      "must redirect to the missing data page when validation returns Left(Call)" in {
        ...
        val application = applicationBuilder(userAnswers = Some(emptyUserAnswers))
          .overrides(
            bind[Navigator].qualifiedWith(Names.named("agentsSh03")).toInstance(fakeNavigatorWithValidatorOutcome(Left(fakeMissingDataCall)))
          )
          .build()
      ...
      "must redirect to the error page when validation returns Right(false)" in {
        val application = applicationBuilder(userAnswers = Some(emptyUserAnswers))
          .overrides(
            bind[Navigator].qualifiedWith(Names.named("agentsSh03")).toInstance(fakeNavigatorWithValidatorOutcome(Right(false)))
          )
          .build()
    ...
    "onSubmit" - {
      ...
      val completeUserAnswers =
        emptyUserAnswers
          ...
          .set(AgentReferencePage, AgentReference(Some("Ref-1234"))).success.value
