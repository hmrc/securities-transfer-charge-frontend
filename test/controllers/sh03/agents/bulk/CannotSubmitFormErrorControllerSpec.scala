...
import uk.gov.hmrc.securitiestransferchargefrontend.views.html.sh03.agents.bulk.CannotSubmitFormErrorView
...
class CannotSubmitFormErrorControllerSpec extends SpecBase {
  ...
  "CannotSubmitFormError Controller" - {
    ...
    "must redirect to Journey Recovery for a GET if no existing data is found" in {
      ...
      val application = applicationBuilder(userAnswers = None, affinityGroup = agentAffinity)
        .overrides(bind[Navigator].qualifiedWith("agentsSh03").toInstance(getNavigator))
        .build()
