...
class CompanyDetailsControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val companyDetailsRoute: String = sh03AgentBulkRoutes.CompanyDetailsController.onPageLoad(NormalMode).url

  val validAnswer: CompanyDetails = CompanyDetails("Test Company Ltd", "AB123456")
  ...
  "CompanyDetailsController" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(testUserId, testGroupIdentifier, submissionId)
        .set(BulkCompanyDetailsPage, validAnswer).success.value
