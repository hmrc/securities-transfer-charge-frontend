...
class DetailsOfThisSharePurchaseControllerSpec extends SpecBase with MockitoSugar {
  ...
  val detailsOfThisSharePurchase: DetailsOfThisSharePurchase = DetailsOfThisSharePurchase(numberOfShares = 25,
    typeOfShares = "stocks",
  ...
  "DetailsOfThisSharePurchase Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(detailsOfThisSharePurchase), NormalMode, testBackLinkRoute)(request, messages(application)).toString
      }
    }
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request = {
          FakeRequest(POST, detailsOfThisSharePurchaseRoute)
