...
class StfSellerAddressControllerSpec extends SpecBase with MockitoSugar {
  ...
  "AddressController" - {
    ...
    "onPageLoad should redirect to ALF" in {
      ...
      running(application) {
        val request = FakeRequest(GET, agentSingleRoutes.StfSellerAddressController.onPageLoad(NormalMode).url)
    ...
    "onReturn should retrieve address and redirect" in {
      ...
      running(application) {
        val request = FakeRequest(GET, agentSingleRoutes.StfSellerAddressController.onReturn("addressId").url)
        val result = route(application, request).value

        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual agentSingleRoutes.ConnectedPersonsController.onPageLoad(NormalMode).url
      }
    }
  }
}
