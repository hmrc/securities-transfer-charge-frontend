...
class NameOfSellerControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val nameOfSellerRoute: String = orgSingleRoutes.NameOfSellerController.onPageLoad(NormalMode).url
  ...
  "NameOfSeller Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(testUserId, testGroupIdentifier, submissionId,STF).set(NameOfSellerPage, "answer").success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value mustEqual orgSingleRoutes.StfSellerAddressController.onPageLoad(NormalMode).url
      }
    }
