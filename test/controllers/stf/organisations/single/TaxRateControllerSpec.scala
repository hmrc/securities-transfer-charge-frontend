...
class TaxRateControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val taxRateRoute: String = orgSingleRoutes.TaxRateController.onPageLoad(NormalMode).url
  ...
  "TaxRate Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(testUserId, testGroupIdentifier, submissionId,STF).set(TaxRatePage, TaxRate.values.head).success.value
