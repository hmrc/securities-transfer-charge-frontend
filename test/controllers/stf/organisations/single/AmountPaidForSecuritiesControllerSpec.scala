...
import uk.gov.hmrc.securitiestransferchargefrontend.forms.stf.organisations.AmountPaidForSecuritiesFormProvider
...
import uk.gov.hmrc.securitiestransferchargefrontend.views.html.stf.organisations.single.AmountPaidForSecuritiesView
...
class AmountPaidForSecuritiesControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val amountPaidForSecuritiesRoute: String = orgSingleRoutes.AmountPaidForSecuritiesController.onPageLoad(NormalMode).url
  ...
  "AmountPaidForSecurities Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(testUserId, testGroupIdentifier, submissionId,STF).set(AmountPaidForSecuritiesPage, validAnswer).success.value
    ...
    "must redirect to the next page when valid data is submitted and connected persons is true" in {
      ...
      running(application) {
        ...
        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual orgSingleRoutes.TotalMarketValueController.onPageLoad(NormalMode).url
      }
    }
