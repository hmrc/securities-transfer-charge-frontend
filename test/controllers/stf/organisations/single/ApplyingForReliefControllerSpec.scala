...
import uk.gov.hmrc.securitiestransferchargefrontend.pages.stf.individuals.single.ApplyingForReliefPage
import uk.gov.hmrc.securitiestransferchargefrontend.views.html.stf.organisations.single.ApplyingForReliefView
...
class ApplyingForReliefControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val applyingForReliefRoute: String = orgSingleRoutes.ApplyingForReliefController.onPageLoad(NormalMode).url
  ...
  "ApplyingForRelief Controller" - {
    ...
    "must redirect to the WhatReliefAreYouApplyingFor page when yes is selected" in {
      ...
      running(application) {
        ...
        redirectLocation(result).value mustEqual orgSingleRoutes.WhatReliefAreYouApplyingForController.onPageLoad(NormalMode).url
      }
    }
    ...
    "must redirect to the SecuritiesTarget page when no is selected" in {
      ...
      running(application) {
        ...
        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual orgSingleRoutes.SecuritiesTargetController.onPageLoad(NormalMode).url
      }
    }
