...
class StfSellerAddressControllerSpec extends SpecBase with MockitoSugar {
  ...
  "AddressController" - {
    ...
    "onPageLoad should redirect to ALF" in {
      ...
      running(application) {
        val request = FakeRequest(GET, orgRoutes.StfSellerAddressController.onPageLoad(NormalMode).url)
    ...
    "onReturn should retrieve address and redirect" in {
      ...
      running(application) {
        val request = FakeRequest(GET, orgRoutes.StfSellerAddressController.onReturn("addressId",NormalMode).url)
        val result = route(application, request).value

        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual orgRoutes.ConnectedPersonsController.onPageLoad(NormalMode).url
      }
    }
  }
}
