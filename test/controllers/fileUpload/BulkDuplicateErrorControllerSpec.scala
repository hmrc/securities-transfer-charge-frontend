...
class BulkDuplicateErrorControllerSpec extends SpecBase {

  "BulkDuplicateErrorController" - {
    ...
    Seq(JourneyType.STF, JourneyType.SH03).foreach { journeyType =>
      ...
      s"$journeyType must return OK and the correct view for a GET" in {
        ...
        running(application) {
          ...
          val request = FakeRequest(GET, routes.BulkDuplicateErrorController.onPageLoad(journeyType).url)
          val result = route(application, request).value
          val view = application.injector.instanceOf[BulkDuplicateErrorView]
