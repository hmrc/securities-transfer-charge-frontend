...
class FileTypeErrorControllerSpec extends SpecBase {

  "FileTypeErrorController" - {
    ...
    Seq(JourneyType.STF, JourneyType.SH03).foreach { journeyType =>
      ...
      s"$journeyType must return OK and the correct view for a GET" in {
        ...
        running(application) {
          ...
          val request = FakeRequest(GET, routes.FileTypeErrorController.onPageLoad(journeyType).url)
          val result = route(application, request).value
          val view = application.injector.instanceOf[FileTypeErrorView]
