...
class CheckYourAnswersViewSpec extends ViewBaseSpec {
  ...
  private val dummyViewModel = CheckYourAnswersViewModel.fromSummaryLists(
    yourDetails = SummaryListViewModel(rows = Seq.empty),
    buyerDetails = SummaryListViewModel(rows = Seq.empty),
  ...
  object ExpectedContent {
    ...
    val printLinkText: String = "print this page"

    val declarationHeading: String = messages(CheckYourAnswersViewModel.MessageKeys.declarationHeading)
    val declarationConfirm: String = messages(CheckYourAnswersViewModel.MessageKeys.declarationConfirm)
  ...
  "The CheckYourAnswersView" - {
    ...
    "display the print page section correctly" in {
      val printSectionHeading = cyaView.select("h2:contains(" + ExpectedContent.printHeading + ")")
    ...
    "display the declaration section correctly" - {
      ...
      "with the confirmation body text" in {
        cyaView.select(s"p.govuk-body:contains(${ExpectedContent.declarationConfirm})").size() mustBe 1
      }

      "with the correct bullet points" in {
        val bullets = cyaView.select("ul.govuk-list--bullet li")
        bullets.text() must include(ExpectedContent.declarationBullet1)
        bullets.text() must include(ExpectedContent.declarationBullet2)
      }
    }
