...
class TaxRateViewSpec extends ViewBaseSpec {
  ...
  object ExpectedContent {
    val title: String = messages("taxRate.title")
    val caption: String = messages("transfer.details.caption")
    val heading: String = messages("taxRate.heading")
    val paragraph: String = messages("agent.taxRate.p1")
    val paragraph2: String = messages("agent.taxRate.p2")
  ...
  "The TaxRateView" - {
    ...
    "when rendered without errors" - {
      ...
      "have the first paragraph" in {
        taxRateView.select(".govuk-body").first().text() mustBe ExpectedContent.paragraph
      }      
      
      "have the second paragraph" in {
        taxRateView.select(".govuk-body").last().text() mustBe ExpectedContent.paragraph2
      }
