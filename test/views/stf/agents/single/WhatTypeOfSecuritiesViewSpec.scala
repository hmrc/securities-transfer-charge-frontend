...
class WhatTypeOfSecuritiesViewSpec extends ViewBaseSpec {
  ...
  object ExpectedContent {
    val title: String = messages("agent.whatTypeOfSecurities.title")
    val caption: String = messages("transfer.details.caption")
    val heading: String = messages("agent.whatTypeOfSecurities.heading")
    val saveAndContinue: String = messages("site.save-and-continue.button")
    val saveAndReturn: String = messages("site.save-and-return.button")
    val sharesRadio: String =  messages("agent.whatTypeOfSecurities.shares")
    val otherRadio: String =  messages("agent.whatTypeOfSecurities.other")
  }
  ...
  "WhatTypeOfSecuritiesView" - {
      ...
      "have the correct radio buttons" in {
        doc.select(".govuk-radios").first().text() must include(ExpectedContent.sharesRadio)
        doc.select(".govuk-radios").last().text() must include(ExpectedContent.otherRadio)
      }
