...
package services.stf

import base.SpecBase
import org.mockito.Mockito.when
import org.scalatestplus.mockito.MockitoSugar
import uk.gov.hmrc.securitiestransferchargefrontend.models.{Relief, ReliefsDataSource}
import uk.gov.hmrc.securitiestransferchargefrontend.models.stf.{DetailsOfThisTransfer, TaxRate}
...
class TaxDueCalculationServiceSpec extends SpecBase with MockitoSugar {
  ...
  "TaxDueCalculationService" - {
    ...
    "calculateTaxDue" - {
      "must calculate tax at 0.5% for shares when amount paid is higher" in {
        val detailsOfTransfer = DetailsOfThisTransfer(100, "Ordinary", BigDecimal("10000"), Some(BigDecimal("8000")))
      ...
      "must calculate tax at 0.5% for shares when market value is higher" in {
        val detailsOfTransfer = DetailsOfThisTransfer(100, "Ordinary", BigDecimal("8000"), Some(BigDecimal("10000")))
        val userAnswers = emptyUserAnswers
          .set(DetailsOfThisTransferPage, detailsOfTransfer).success.value
          .set(TaxRatePage, TaxRate.HalfPercent).success.value
          .set(ApplyingForReliefPage, false).success.value
      ...
      "must calculate tax at 1.5% for shares" in {
        val detailsOfTransfer = DetailsOfThisTransfer(100, "Ordinary", BigDecimal("10000"), Some(BigDecimal("10000")))
        val userAnswers = emptyUserAnswers
          .set(DetailsOfThisTransferPage, detailsOfTransfer).success.value
          .set(TaxRatePage, TaxRate.OneAndHalfPercent).success.value
          .set(ApplyingForReliefPage, false).success.value
      ...
      "must calculate tax for other securities when amount paid is higher" in {
        val userAnswers = emptyUserAnswers
          ...
          .set(TotalMarketValuePage, BigDecimal("4000")).success.value
          .set(TaxRatePage, TaxRate.HalfPercent).success.value
          .set(ApplyingForReliefPage, false).success.value
      ...
      "must calculate tax for other securities when market value is higher" in {
        val userAnswers = emptyUserAnswers
          .set(AmountPaidForSecuritiesPage, BigDecimal("4000")).success.value
          .set(TotalMarketValuePage, BigDecimal("5000")).success.value
          .set(TaxRatePage, TaxRate.HalfPercent).success.value
          .set(ApplyingForReliefPage, false).success.value
      ...
      "must round to 2 decimal places correctly" in {
        val detailsOfTransfer = DetailsOfThisTransfer(100, "Ordinary", BigDecimal("10001"), Some(BigDecimal("10001")))
        val userAnswers = emptyUserAnswers
          .set(DetailsOfThisTransferPage, detailsOfTransfer).success.value
          .set(TaxRatePage, TaxRate.HalfPercent).success.value
          .set(ApplyingForReliefPage, false).success.value
