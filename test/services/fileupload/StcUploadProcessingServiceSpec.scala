...
class StcUploadProcessingServiceSpec extends SpecBase with MockitoSugar {
  ...
  "StcUploadProcessingService.process" - {
    ...
    "parse then validate the uploaded file stream" in {
      val rowStream = Seq(parsedRow).iterator

      when(
        stcUploadParsingService.withValidatedStream[StcFileValidationResponse](
          eqTo(uploadedFile),
