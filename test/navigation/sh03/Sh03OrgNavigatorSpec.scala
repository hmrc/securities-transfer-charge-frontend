  ...
  "Sh03OrgNavigator" - {
    ...
    "in Normal mode" - {
      ...
      "in Normal mode" - {
        ...
        "must go from the HowToNotifyAboutShareBuybackPage to OrgCompanyDetailsPage when one at a time is selected" in {
        ...
        "must go from OrgCompanyDetailsPage to ReasonForPurchasePage" in {
          val answers = emptyUserAnswers.set(OrgCompanyDetailsPage, "12345678").get
          val result = navigator.nextPage(OrgCompanyDetailsPage, NormalMode, answers)(fakeRequest)
          whenReady(result) { res =>
        ...
        "must go from DetailsOfThisSharePurchasePage to MaximumAmountPaidPage if company is a PLC" in {
          val answers = emptyUserAnswers.set(DetailsOfThisSharePurchasePage, purchaseDetails).get
...
//         TODO AFTER THE CR TICKET HAS BEEN IMPLEMENTED
//        "must go from DetailsOfThisSharePurchasePage to ChargingPointPage if company is not a PLC" in {
...
//        }
    ...
    "in Previous Pages" - {
      ...
      "must go from the OrgCompanyDetailsPage to HowToNotifyAboutShareBuybackPage" in {
        val result = navigator.previousPage(OrgCompanyDetailsPage, NormalMode, emptyUserAnswers)
        result mustBe sh03OrgRoutes.HowToNotifyAboutShareBuybackController.onPageLoad()
      }

      "must go from the ReasonForPurchasePage to OrgCompanyDetailsPage" in {
