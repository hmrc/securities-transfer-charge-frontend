...
class Sh03AgentNavigatorSpec extends SpecBase with ScalaFutures {
  ...
  "Sh03AgentNavigator" - {
    ...
    "in Normal mode" - {
      ...
      "must go from a page that doesn't exist in the route map to default page" in {
        ...
        val result = navigator.nextPage(UnknownPage, NormalMode, UserAnswers(testUserId, testGroupIdentifier, submissionId,SH03))(fakeRequest)
      ...
      "must go from any page to the dashboard page if isReturn is true" in {
        ...
        val result = navigator.nextPage(AnyPage, NormalMode, UserAnswers(testUserId, testGroupIdentifier, submissionId,SH03), true)(fakeRequest)
        whenReady(result) { res =>
  ...
  "in Check mode" - {
    ...
    "must go from a page that doesn't exist in the edit route map to CheckYourAnswers" in {
      ...
      case object UnknownPage extends Page
      val result = navigator.nextPage(UnknownPage, CheckMode, UserAnswers(testUserId, testGroupIdentifier, submissionId,SH03))(fakeRequest)
      whenReady(result) { res =>
        res mustBe cyaPage
      }
    }
