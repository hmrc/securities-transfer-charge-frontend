  ...
  "StfAgentNavigator" - {
    ...
    "in Normal mode" - {
      ...
      "must go from the NameOfBuyerPage to StfBuyersAddressPage" in {
        ...
        val result = navigator.nextPageCall(NameOfBuyerPage, NormalMode, answers)
        whenReady(result) { res =>
      ...
      "must go from PurchasingSharesPage to DetailsOfThisTransferPage if true" in {
        ...
        val result = navigator.nextPageCall(PurchasingSharesPage, NormalMode, answers)
        whenReady(result) { res =>
      ...
      "must go from AmountPaidForSecuritiesPage to TotalMarketValuePage when connected persons is true" in {
        ...
        val result = navigator.nextPageCall(AmountPaidForSecuritiesPage, NormalMode, answers)
    ...
    "in Previous Pages" - {
      ...
      "must go from the AgentReferencePage to HowToNotifyAboutSecuritiesTransferPage if it is in single journey" in {
        val answers = emptyUserAnswers.set(HowToNotifyAboutSecuritiesTransferPage, HowToNotifyAboutSecuritiesTransfer.OneAtATime).get
        val result = navigator.previousPageCall(AgentReferencePage, NormalMode, answers)
        result mustBe agentRoutes.HowToNotifyAboutSecuritiesTransferController.onPageLoad()
      }
      "must go from the AgentReferencePage to FileUploadPage if it is in bulk journey" in {
        val answers = emptyUserAnswers.set(HowToNotifyAboutSecuritiesTransferPage, HowToNotifyAboutSecuritiesTransfer.MoreThanOneAtATime).get
        val result = navigator.previousPageCall(AgentReferencePage, NormalMode, answers)
      ...
      "must go from the NameOfBuyerPage to AgentReferencePage" in {
        val result = navigator.previousPageCall(NameOfBuyerPage, NormalMode, emptyUserAnswers)
      ...
      "must go from the PurchasingSharesPage to TaxRatePage" in {
        val result = navigator.previousPageCall(PurchasingSharesPage, NormalMode, emptyUserAnswers)
