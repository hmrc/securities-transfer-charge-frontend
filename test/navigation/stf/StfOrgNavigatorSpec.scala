  ...
  "StfOrgNavigator" - {
    ...
    "in Normal mode" - {
      ...
      "must go from a page that doesn't exist in the route map to default page" in {
        ...
        val result = navigator.nextPage(UnknownPage, NormalMode, UserAnswers(testUserId, testGroupIdentifier, submissionId,STF))(fakeRequest)
      ...
      "must go from any page to the dashboard page if isReturn is true" in {
        ...
        val result = navigator.nextPage(AnyPage, NormalMode, UserAnswers(testUserId, testGroupIdentifier, submissionId,STF), true)(fakeRequest)
        whenReady(result) { res =>
      ...
      "must go from the NameOfSellerPage to StfSellerAddressPage" in {
        ...
        whenReady(result) { res =>
          res mustBe orgSingleRoutes.StfSellerAddressController.onPageLoad(NormalMode)
        }
      }
      ...
      "must go from the TaxRatePage to PurchasingSharesController" in {
        ...
        whenReady(result) { res =>
          res mustBe orgSingleRoutes.PurchasingSharesController.onPageLoad(NormalMode)
        }
      }
    ...
    "in Check mode" - {
      ...
      "must go from a page that doesn't exist in the edit route map to CheckYourAnswers" in {
        ...
        case object UnknownPage extends Page
        val result = navigator.nextPage(UnknownPage, CheckMode, UserAnswers(testUserId, testGroupIdentifier, submissionId,STF))(fakeRequest)
        whenReady(result) { res =>
          res mustBe stfSingleCyaRoutes.CheckYourAnswersController.onPageLoad()
        }
      }
    ...
    "Previous Pages" - {
      ...
      "must go from the OtherSecuritiesTypePage to PurchasingShares" in {
        ...
        result mustBe orgSingleRoutes.PurchasingSharesController.onPageLoad(NormalMode)
      }
      ...
      "must go from the PurchasingShares to TaxRate" in {
      ...
      "must go from the ConnectedPersonsPage to StfSellerAddress" in {
        ...
        result mustBe orgSingleRoutes.StfSellerAddressController.onPageLoad(NormalMode)
      }
