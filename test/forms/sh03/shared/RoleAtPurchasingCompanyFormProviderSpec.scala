...
class RoleAtPurchasingCompanyFormProviderSpec extends SpecBase {
  ...
  Seq("org", "agent").foreach { affinity =>

    s"RoleAtPurchasingCompanyFormProvider ($affinity)" - {

      val formProvider = new RoleAtPurchasingCompanyFormProvider()
      val form: Form[RoleAtPurchasingCompany] = formProvider(affinity)
      ...
      "must bind valid data for UK Societas with an organ name" in {
        val data = Map(
          "role" -> "ukSocietas",
          "uksOrgan" -> "Management Board"
        )

        val result = form.bind(data)

        result.value.value mustBe RoleAtPurchasingCompany(
          "ukSocietas",
      ...
      "must fail to bind when no role is selected" in {
        ...
        result.errors must contain(
          FormError(
            "role",
            s"$affinity.sh03.roleAtPurchasingCompany.error.required"
          )
        )
      }
      ...
      "must fail to bind when UK Societas is selected but the organ name is left blank" in {
        ...
        result.errors must contain(
          FormError(
            "uksOrgan",
            s"$affinity.sh03.roleAtPurchasingCompany.uksOrgan.error.required"
          )
        )
      }
      ...
      "must fail to bind when UK Societas organ name exceeds 100 characters" in {
        ...
        result.errors must contain(
          FormError(
            "uksOrgan",
            s"$affinity.sh03.roleAtPurchasingCompany.uksOrgan.error.length"
          )
        )
      }
    }
