...
class AmountPaidForSecuritiesFormProviderSpec extends CurrencyFieldBehaviours {
  ...
  ".value" - {
    ...
    behave like currencyField(
      form,
      fieldName,
      nonNumericError     = FormError(fieldName, "amountPaidForSecurities.error.nonNumeric"),
      invalidNumericError = FormError(fieldName, "amountPaidForSecurities.error.invalidNumeric")
    )
    ...
    behave like currencyFieldWithMaximum(
      ...
      fieldName,
      maximum,
      FormError(fieldName, "amountPaidForSecurities.error.aboveMaximum", Seq(currencyFormat(maximum)))
    )

    behave like mandatoryField(
      form,
      fieldName,
      requiredError = FormError(fieldName, "amountPaidForSecurities.error.required")
    )
  }
}
