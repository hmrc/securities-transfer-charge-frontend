...
import uk.gov.hmrc.securitiestransferchargefrontend.pages.sh03.single.ApplyingForReliefPage
    ...
    answers.get(ApplyingForReliefPage).map {
      answer =>
        ...
        SummaryListRowViewModel(
          key     = "agent.sh03.applyingForRelief.checkYourAnswersLabel",
          value   = ValueViewModel(value),
          actions = Seq(
            ActionItemViewModel("site.change", routes.ApplyingForReliefController.onPageLoad(CheckMode).url)
              .withVisuallyHiddenText(messages("agent.sh03.applyingForRelief.change.hidden"))
          )
        )
    }
}
