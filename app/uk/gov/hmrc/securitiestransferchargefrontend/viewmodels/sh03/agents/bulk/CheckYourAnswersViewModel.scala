...
case class CheckYourAnswersViewModel(
                                      ...
                                      declarationSummary: SummarySection
                                    )
...
object CheckYourAnswersViewModel {
  ...
  object MessageKeys {
    ...
    val saveAndReturn = "checkYourAnswers.saveAndReturn"
  ...
  def agentBulkSummaryLists(
    ...
    val declarationSummary = SummarySection(MessageKeys.declarationHeading, declarationSummaryList)

    CheckYourAnswersViewModel(sections, Some(taxDueSummaryRows), totalTaxDue, Some(taxDueDate), declarationSummary)
  }
...
object Formatter {
  ...
  def formatAmount(amount: BigDecimal): String = f"$amount%,.0f"
}