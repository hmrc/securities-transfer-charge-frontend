...
object MaximumAmountPaidSummary {

  def row(answers: UserAnswers)(implicit messages: Messages): Option[SummaryListRow] =
    answers.get(MaximumAmountPaidPage).map {
      answer =>

        SummaryListRowViewModel(
          key = "agent.sh03.maximumAmountPaid.checkYourAnswersLabel",
          value = ValueViewModel(currencyFormat(answer)),
          actions = Seq(
            ActionItemViewModel("site.change", routes.MaximumAmountPaidController.onPageLoad(CheckMode).url)
              .withVisuallyHiddenText(messages("agent.sh03.maximumAmountPaid.change.hidden"))
          )
        )
    }
}