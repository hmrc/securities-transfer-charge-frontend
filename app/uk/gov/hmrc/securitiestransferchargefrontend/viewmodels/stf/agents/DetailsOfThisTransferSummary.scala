...
object DetailsOfThisTransferSummary {
  ...
  def rows(answers: UserAnswers, showMarketValue: Boolean = true)(implicit messages: Messages): Option[Seq[SummaryListRow] ]= {
    answers.get(DetailsOfThisTransferPage).map { details =>
      
      val numberOfShares = details.numberOfShares.toString
      val typeOfShares = details.typeOfShares
      val amountPaid = details.amountPaid
      val marketValue = details.marketValue

      val baseRows = Seq(
        SummaryListRowViewModel(
          key = "checkYourAnswers.numberOfShares",
          value = ValueViewModel(HtmlFormat.escape(numberOfShares).toString),
          actions = Seq(
        ...
        SummaryListRowViewModel(
          ...
          value = ValueViewModel(HtmlFormat.escape(currencyFormat(amountPaid)).toString),
          actions = Seq(
            ActionItemViewModel("site.change", routes.DetailsOfThisTransferController.onPageLoad(CheckMode).url)
      ...
      val marketValueRow = if (showMarketValue) {
        Seq(SummaryListRowViewModel(
          ...
          value = ValueViewModel(HtmlFormat.escape(marketValue.map(currencyFormat).getOrElse("")).toString),
          actions = Seq(
            ActionItemViewModel("site.change", routes.DetailsOfThisTransferController.onPageLoad(CheckMode).url)
