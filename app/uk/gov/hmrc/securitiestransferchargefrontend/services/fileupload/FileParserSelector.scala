...
class FileParserSelector @Inject()(
                                    ...
                                    xlsxFileParser: ExcelFileParser
                                  ) {
  ...
  def select(mimeType: String): Either[FileParseError, FileParser] =
    if (csvMimeTypes.contains(mimeType)) {
      Right(csvFileParser)
    } else if (xlsxMimeTypes.contains(mimeType)) {
      Right(xlsxFileParser)
    } else {
      Left(UnsupportedMimeType(mimeType))
    }
}