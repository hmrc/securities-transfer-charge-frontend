...
class NavigationHelper(defaultPage: Call):

  /*
   * Used to navigate when the destination depends on a UserAnswer being present for the page,
   * but the value doesn't matter.
   */
  protected[navigation] def dataRequired[A: Reads](page: Page & Gettable[A], userAnswers: UserAnswers, success: Call): Call =
   ...
   */
  protected[navigation] def dataDependent[A: Reads](page: Page & Gettable[A], userAnswers: UserAnswers)(f: A => Call): Call =
