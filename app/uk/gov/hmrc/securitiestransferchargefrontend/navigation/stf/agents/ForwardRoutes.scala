...
import uk.gov.hmrc.securitiestransferchargefrontend.pages.stf.single.{AgentReferencePage, NameOfSellerPage, StfBuyersAddressPage, StfSellerAddressPage}
...
class ForwardRoutes(answerPersistenceService: AnswerPersistenceService,
  ...
  def forwardRoutes(page: Page)(implicit hc: HeaderCarrier): UserAnswers => Future[Call] = page match {
    ...
    case AgentReferencePage => userAnswers => dataRequired(AgentReferencePage, userAnswers, agentSingleRoutes.AddressController.onPageLoad())
    ...
    case StfSellerAddressPage => userAnswers => dataRequired(StfSellerAddressPage, userAnswers, defaultPage)
