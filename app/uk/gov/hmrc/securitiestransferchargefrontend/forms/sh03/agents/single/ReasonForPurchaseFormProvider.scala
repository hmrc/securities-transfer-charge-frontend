...
class ReasonForPurchaseFormProvider @Inject() extends Mappings {

  def apply(): Form[ReasonForPurchase] =
    Form(
      "value" -> enumerable[ReasonForPurchase]("reasonForPurchase.error.required")
    )
}
