...
class SecuritiesTargetFormProvider @Inject() extends Mappings {
   ...
   def apply(affinityKey:String): Form[SecuritiesTarget] = Form(
     mapping(
      "businessName" -> text(s"fileUpload.${affinityKey}.securitiesTarget.error.businessName.required")
        .verifying(maxLength(160, s"fileUpload.${affinityKey}.securitiesTarget.error.businessName.length")),
       "crn" -> validatedOptionalText(s"fileUpload.${affinityKey}.securitiesTarget.error.crn.length", 8)
         .verifying(s"fileUpload.${affinityKey}.securitiesTarget.error.crn.invalid", crn => crn.forall(s => s.matches("^[a-zA-Z0-9]{8}$")))
