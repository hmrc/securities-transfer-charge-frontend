...
class TotalMarketValueFormProvider @Inject() extends Mappings {
  ...
  def apply(): Form[BigDecimal] =
    Form(
      "value" -> currency(
        "org.totalMarketValue.error.required",
        "org.totalMarketValue.error.invalidNumeric",
        "org.totalMarketValue.error.nonNumeric"
      )
      .verifying(maximumCurrency(Int.MaxValue, "org.totalMarketValue.error.aboveMaximum"))
    )
}
