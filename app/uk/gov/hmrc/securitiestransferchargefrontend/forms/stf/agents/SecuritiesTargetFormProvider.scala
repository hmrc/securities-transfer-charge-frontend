...
class SecuritiesTargetFormProvider @Inject() extends Mappings {
   ...
   def apply(): Form[SecuritiesTarget] = Form(
     mapping(
      "businessName" -> text("agent.securitiesTarget.error.businessName.required")
        .verifying(maxLength(160, "agent.securitiesTarget.error.businessName.length")),
       "crn" -> validatedOptionalText("agent.securitiesTarget.error.crn.length", 8)
         .verifying("agent.securitiesTarget.error.crn.invalid", crn => crn.forall(s => s.matches("^[a-zA-Z0-9]{8}$")))
