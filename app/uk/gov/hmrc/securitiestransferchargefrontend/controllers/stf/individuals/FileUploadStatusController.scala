...
class FileUploadStatusController @Inject()(
  ...
  def check(reference: String): Action[AnyContent] = stcAuthEnrolled.async { implicit request =>
    ...
    upscanJourneyRepository.find(reference).map {
      ...
      case None => Ok(Json.obj("fileStatus" -> "failed"))
    }
  }
}
