  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = auth.async {
    implicit request =>
      val returnUrl = mode match {
        case NormalMode => config.alfStfBuyersContinueUrl
        case CheckMode => config.alfStfBuyersContinueChangeUrl
      }
      super.pageLoad(config.buyersAlfConfigFileLocation, returnUrl)
  }
