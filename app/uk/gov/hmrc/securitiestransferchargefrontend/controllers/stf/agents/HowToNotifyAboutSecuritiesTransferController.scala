...
package uk.gov.hmrc.securitiestransferchargefrontend.controllers.stf.agents
...
class HowToNotifyAboutSecuritiesTransferController @Inject()(
  ...
  lazy val backLinkCall: Mode => UserAnswers => Call =
    mode => userAnswers => navigator.previousPageCall(HowToNotifyAboutSecuritiesTransferPage, mode, userAnswers)
      ...
      form.bindFromRequest().fold(
        ...
        howToNotify =>
          for {
            ...
            nextPage       <- navigator.nextPageCall(HowToNotifyAboutSecuritiesTransferPage, NormalMode, updatedAnswers, isReturn(request))
