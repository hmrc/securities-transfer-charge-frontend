...
class NameOfBuyerController @Inject()(
                                        override val messagesApi: MessagesApi,
                                        @Named("agents") navigator: Navigator,
                                        stcAuthEnrolled: StcAuthEnrolledAction,
                                        getData: StcDataRetrievalAction,
                                        requireData: StcDataRequiredAction,
                                        formProvider: NameOfBuyerFormProvider,
                                        val controllerComponents: MessagesControllerComponents,
                                        view: NameOfBuyerView
                                    )(implicit ec: ExecutionContext) extends FrontendBaseController with I18nSupport {

  val form: Form[String] = formProvider()

  lazy val backLinkCall: Mode => UserAnswers => Call =
    mode => userAnswers => navigator.previousPageCall(NameOfBuyerPage, mode, userAnswers)

  def onPageLoad(mode: Mode): Action[AnyContent] = (stcAuthEnrolled andThen getData andThen requireData) {
    implicit request =>

      val preparedForm = request.userAnswers.get(NameOfBuyerPage) match {
        case None => form
      ...
      form.bindFromRequest().fold(
        ...
        value =>
          for {
            ...
            nextPage       <- navigator.nextPageCall(NameOfBuyerPage, mode, updatedAnswers, isReturn(request))
