...
class StfSellerAddressController @Inject()(val controllerComponents: MessagesControllerComponents,
  ...
  def onPageLoad(mode: Mode = NormalMode): Action[AnyContent] = auth.async {
    implicit request =>
      val continueUrl = s"${config.alfAgentSellerContinueUrl}?mode=${mode.toString}"
      super.pageLoad(config.agentSellerAlfConfigFileLocation, continueUrl)
  }

  def onReturn(addressId: String, mode: Mode = NormalMode): Action[AnyContent] = (auth andThen getData andThen requireData).async {
    implicit request =>
      for {
