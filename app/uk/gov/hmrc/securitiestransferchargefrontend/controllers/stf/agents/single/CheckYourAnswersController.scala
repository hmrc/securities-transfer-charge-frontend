...
class CheckYourAnswersController @Inject()(
  ...
  def onPageLoad(): Action[AnyContent] = (stcAuthEnrolled andThen getData andThen requireData).async {
    implicit request =>
      ...
      navigator.userAnswersValidator.validate(request.userAnswers).map {
        case Left(redirectCall) =>
          Redirect(redirectCall)
        case Right(true) =>
          buildCheckYourAnswersPage(request.userAnswers)
        case Right(false) =>
          Redirect(navigator.errorPage(CheckYourAnswersPage))
      }
  }
  ...
  private def buildCheckYourAnswersPage(userAnswers: UserAnswers)(implicit messages: Messages, lang: Lang, request: StcDataRequest[AnyContent]): play.api.mvc.Result = {
    val summaryLists = Seq(
      StfAgentRowBuilder.buildYourDetailsRows(userAnswers)(messages),
      StfAgentRowBuilder.buildBuyerDetailsRows(userAnswers)(messages),
      StfAgentRowBuilder.buildSellerDetailsRows(userAnswers)(messages),
      StfAgentRowBuilder.buildTransferDetailsRows(userAnswers)(messages)
    ...
    val viewModel = CheckYourAnswersViewModel.fromAgentSummaryLists(
