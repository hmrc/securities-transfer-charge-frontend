...
import uk.gov.hmrc.securitiestransferchargefrontend.forms.sh03.organisations.single.ReasonForPurchaseFormProvider
...
class ReasonForPurchaseController @Inject()(
                                       ...
                                       formProvider: ReasonForPurchaseFormProvider,
                                       val controllerComponents: MessagesControllerComponents,
                                       view: ReasonForPurchaseView
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] =  (stcAuthEnrolled andThen getData andThen requireData) {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(ReasonForPurchasePage) match {
        case None => form
